...
public partial class FPCharacter : CharacterBody3D
{
    ...
    public override void _PhysicsProcess(double delta)
    {
		// Add gravity.
		if (!IsOnFloor())
			AddGracityForce(GravityForce, (float)delta);
		...
		if(IsOnFloor())
		{
			_InternalVelocity.x = _MoveDir.x * _SpeedMul;
			_InternalVelocity.z = _MoveDir.z * _SpeedMul;
		}
	...
	private void Jump()
	{
		_InternalVelocity.y = _InternalJumpForce;
	}

	private void AddGracityForce(float force, float delta)
	{
		_InternalVelocity.y -= force * delta;
	}
