...
namespace JC.Footsteps
{
	...
	public partial class Footsteps: Node3D
	{
		...
		/// <summary> Enable putch fx. </summary>
		...
		private void PlaySurfaceClips(bool panner)
		{
			if(surfaces.Count > 0)
			{
				...
				{
					if(surface.Exist(_CurrentSurfaceTexture))
					{
						...
						surface.clips[i] = surface.clips[0];
						surface.clips[0] = AudioPlayer.Stream;
					}
				}
			}
			else
		...
		private void PlayDefaultClips(bool panner)
		{
			...
			DefaultClips.clips[i] = DefaultClips.clips[0];
			DefaultClips.clips[0] = AudioPlayer.Stream;
		}
	}
}
